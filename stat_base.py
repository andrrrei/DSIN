...
data_bdns = data_bdns.rename(columns={'Студенческий' : 'Номер студенческого','Срок действия' : 'Истечение документов'})
data_bdns['Статус'] = data_bdns['Статус'].replace(['', 'Ок'],['В обработке', 'Все в порядке'])

# Дополнение таблицы формой ответов

ws_ans = gc.open_by_key("1fZhfUDWSGGr6uHQVdMpA1O2KNX32uXpKe8hMMNkoeMM") # Ответы на форму
data = pd.DataFrame(ws_ans.get_worksheet(0).get_all_records())

data = data[data['Номер студенческого билета'] != ''] # оставляем лишь тех, у кого есть номер студака

list_stud = list(data_bdns['Номер студенческого'])

data = data[~data['Номер студенческого билета'].isin(list_stud)] # Этих людей ещё не внесли в базу

data = data[['Номер студенческого билета', 'Курс', 'Статус']]
data = data.rename(columns={'Номер студенческого билета' : 'Номер студенческого'})
data['Статус'] = data['Статус'].replace(['Внести', 'Продлить', '', 'Ошибка'],['В обработке', 'В обработке', 'В обработке', 'В обработке'])
...
search = lambda x: datetime.strptime(x, '%d.%m.%Y') <= datetime.today() if re.search(r"\d{2}.\d{2}.\d{4}", x) else False
data_bdns['tmp'] = data_bdns['Истечение документов'].map(search) # создаём вспомогательный столбец для проверки истечения даты
...
sh = gc.open_by_key('1XYnZHF1nyA4RANVcNYgn1AxKudt0xC8C-XytDrAf8ck') # id итоговой таблицы
